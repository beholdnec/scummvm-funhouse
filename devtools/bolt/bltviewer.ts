...
let DEFAULT_PALETTE: Array<number> = new Array(256)
for (let r = 0; r < 4; r++) {
  for (let g = 0; g < 8; g++) {
    for (let b = 0; b < 4; b++) {
      const i = 8 * 4 * r + 4 * g + b
...
let currentPalette: Array<number> = DEFAULT_PALETTE

function u8hex(x: number): string {
...
type File = { file: number, position: number }
type DirEntry = {
  numResources: number,
  compBufSize: number,
  position: number,
  resourceTable: Array<ResEntry>
}
type ResEntry = {
  type: number,
  compression: number,
...
let bltFile: File = null
let dirTable: Array<DirEntry> = null

function myOpen(path): File {
  return { file: fs.openSync(path, 'r'), position: 0 }
}

function myRead(file: File, numBytes: number): Uint8Array {
  let buf = new Uint8Array(numBytes)
  let numBytesRead = fs.readSync(file.file, buf as Buffer, 0, numBytes, file.position)
  file.position += numBytesRead
  return buf
}

function myReadU8(file: File): number {
  let buf = myRead(file, 1)
  return new DataView(buf.buffer).getUint8(0)
}

function myReadU16(file: File): number {
  let buf = myRead(file, 2)
  // All BLT fields are in big-endian format.
  return new DataView(buf.buffer).getUint16(0)
}

function myReadU32(file: File): number {
  let buf = myRead(file, 4)
  // All BLT fields are in big-endian format.
  return new DataView(buf.buffer).getUint32(0)
}

function mySkip(file: File, numBytes: number) {
  file.position += numBytes
}

function mySeek(file: File, position: number) {
  file.position = position
}

function myTell(file: File): number {
  return file.position
}

function convertIndexedToRgba(dstImageData: ImageData, srcBytes: Uint8Array, palette: Array<number>) {
...
function decodeRL7(dst: Uint8Array, src: Uint8Array, width: number, height: number) {
  let inCursor = 0
  let outX = 0
  let outY = 0
  while (outY < height) {
...
function decompressBoltLZ(dst: Uint8Array, src: Uint8Array) {
  let inCursor = 0
  let outCursor = 0
...
function setDataFields(content: HTMLElement, dataFields: Array<any>) {
  for (const [name, val] of Object.entries(dataFields)) {
    (content.querySelector(`[data-field="${name}"]`) as HTMLElement).innerText = val
  }
}
...
function emitDataFields(dataView: DataView, tableEl: Element, fields: Array<any>) {
  tableEl.innerHTML = ""
...
function emitDataFieldsArray(dataView: DataView, bytesPerItem: number, tableEl: Element, fields: Array<any>) {
  tableEl.innerHTML = ""
...
function openBltFile(path: string) {
  bltFile = null
  dirTable = null
  ...
  if (!Buffer.from(magic.buffer).equals(Buffer.from('BOLT'))) {
...
function openBltSigned16BitValueList(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
function openBltUnsigned16BitValueList(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 6
function openBltResourceList(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 7
function openBltSound(data: Uint8Array) {
...
// Type 8
function openBltImage(data: Uint8Array) {
  ...
  const canvasEl = document.getElementById('blt-image-canvas') as HTMLCanvasElement
...
// Type 10
function openBltPalette(data: Uint8Array) {
...
// Type 11
function openBltColorCycles(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 12
function openBltColorCycleSlot(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 26
function openBltPlane(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 27
function openBltSprite(data: Uint8Array) {
...
// Type 30
function openBltButtonGraphics(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 31
function openBltButtonList(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 32
function openBltScene(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 33
function openBltMainMenu(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 40
function openBltHub(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 41
function openBltHubItem(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
function openBltSlidingPuzzle(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
function openBltParticleDeaths(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
function openBltPotionPuzzle(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
// Type 63
function openBltPotionComboList(data: Uint8Array) {
  const contentEl = document.getElementById('blt-generic-content')
  contentEl.classList.add('is-shown')
...
const PC_RESOURCE_LOADERS: { [type: number]: (data: Uint8Array) => void } = {
...
function openResource(resourceId: number) {
...
document.getElementById('blt-sound-play-btn').addEventListener('click', function (event: MouseEvent) {
  if (bltAudioSource) {
    bltAudioSource.stop()
    bltAudioSource = null
  }
